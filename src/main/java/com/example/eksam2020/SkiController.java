...
public class SkiController {
    ...
    public void lagreMelding(Utover utover, HttpServletResponse response) throws IOException{
        ...
        try {
            db.update(sql,utover.getFornavn(),utover.getEtternavn(),
                    utover.getKlubb(),utover.getEpost(),hash);
        }catch (Exception e){
    ...
    public boolean login(Bruker bruker,HttpServletResponse response)throws IOException{
        ...
        try {
            Utover dbBruker=db.queryForObject(sql, BeanPropertyRowMapper.newInstance(Utover.class),new Object[]{bruker.getEpost()});
            if(sjekkPassord(bruker.getPassord(),dbBruker.getPassord())){
